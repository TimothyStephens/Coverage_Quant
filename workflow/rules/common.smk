...
def get_bbmap_bams(wildcards):
    outbams = []
    for i, row in samples.iterrows():
        outbams.append("results/mapping/{ref_name}/{lib_type}/{sample}.coordSorted.bam".format(ref_name=list(config["ref_path"].keys())[0], sample=row.sample_id, lib_type=row.lib_type))
    outbams = list(set(outbams))
    return outbams
